...
def highlight_differences(chosen, rejected):
    ...
    for line in differ:
        if line.startswith('  '):
            ...
            highlighted_chosen.append(f"<span style='background-color: #66ff66; color: black;'>{line[2:]}</span>")
        elif line.startswith('+ '):
            highlighted_rejected.append(f"<span style='background-color: #ff6666; color: black;'>{line[2:]}</span>")
